...
class Form1 extends Component {
  ...
  addPlayer = (e) => {
    ...
    this.setState({submited: true, player: player})
    ...
    setTimeout(()=>{
      ...
      if (!this.state.formError && this.state.imgUrl) {
        this.props.addPlayer(playerData, playerAdded); // call action with player and callback
      }
  ...
  render() { 
    ...
    return (
      ...
      <div className="form-skills">
        ...
        <p className={ !this.state.imgUrl && this.state.submited ? 'show' : 'hide' }>Plese enter valid img url (jpeg|jpg|gif|png|jpe).</p>
...
const mapStateToProps = (state) => ({
  player: state.playersData.lastPlayerAdded
})

export default connect(mapStateToProps, { addPlayer })(Form1);
